"use strict";
var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    function adopt(value) { return value instanceof P ? value : new P(function (resolve) { resolve(value); }); }
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : adopt(result.value).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
var __generator = (this && this.__generator) || function (thisArg, body) {
    var _ = { label: 0, sent: function() { if (t[0] & 1) throw t[1]; return t[1]; }, trys: [], ops: [] }, f, y, t, g = Object.create((typeof Iterator === "function" ? Iterator : Object).prototype);
    return g.next = verb(0), g["throw"] = verb(1), g["return"] = verb(2), typeof Symbol === "function" && (g[Symbol.iterator] = function() { return this; }), g;
    function verb(n) { return function (v) { return step([n, v]); }; }
    function step(op) {
        if (f) throw new TypeError("Generator is already executing.");
        while (g && (g = 0, op[0] && (_ = 0)), _) try {
            if (f = 1, y && (t = op[0] & 2 ? y["return"] : op[0] ? y["throw"] || ((t = y["return"]) && t.call(y), 0) : y.next) && !(t = t.call(y, op[1])).done) return t;
            if (y = 0, t) op = [op[0] & 2, t.value];
            switch (op[0]) {
                case 0: case 1: t = op; break;
                case 4: _.label++; return { value: op[1], done: false };
                case 5: _.label++; y = op[1]; op = [0]; continue;
                case 7: op = _.ops.pop(); _.trys.pop(); continue;
                default:
                    if (!(t = _.trys, t = t.length > 0 && t[t.length - 1]) && (op[0] === 6 || op[0] === 2)) { _ = 0; continue; }
                    if (op[0] === 3 && (!t || (op[1] > t[0] && op[1] < t[3]))) { _.label = op[1]; break; }
                    if (op[0] === 6 && _.label < t[1]) { _.label = t[1]; t = op; break; }
                    if (t && _.label < t[2]) { _.label = t[2]; _.ops.push(op); break; }
                    if (t[2]) _.ops.pop();
                    _.trys.pop(); continue;
            }
            op = body.call(thisArg, _);
        } catch (e) { op = [6, e]; y = 0; } finally { f = t = 0; }
        if (op[0] & 5) throw op[1]; return { value: op[0] ? op[1] : void 0, done: true };
    }
};
Object.defineProperty(exports, "__esModule", { value: true });
var express_1 = require("express");
var webhook_secrets_1 = require("../services/webhook-secrets");
var db_1 = require("../config/db");
var logger_1 = require("../services/logger");
var router = (0, express_1.Router)();
// Generate new webhook secret for a website
router.post('/:domain/webhook-secret', function (req, res) { return __awaiter(void 0, void 0, void 0, function () {
    var domain, website, secret, error_1, errorDomain;
    return __generator(this, function (_a) {
        switch (_a.label) {
            case 0:
                _a.trys.push([0, 3, , 4]);
                domain = req.params.domain;
                logger_1.logger.debug({
                    message: 'Generating new webhook secret',
                    domain: domain
                }, {
                    component: 'webhook-secrets-controller',
                    event: 'generate_secret'
                });
                return [4 /*yield*/, (0, db_1.getWebsiteByDomain)(domain)];
            case 1:
                website = _a.sent();
                if (!website) {
                    return [2 /*return*/, res.status(404).json({ error: 'Website not found' })];
                }
                // Generate new secret
                logger_1.logger.debug({
                    message: 'Creating/updating webhook secret',
                    domain: domain,
                    websiteId: website.id
                }, {
                    component: 'webhook-secrets-controller',
                    event: 'create_update_secret'
                });
                return [4 /*yield*/, webhook_secrets_1.WebhookSecrets.createOrUpdateSecret(website.id)];
            case 2:
                secret = _a.sent();
                logger_1.logger.info({
                    message: 'Webhook secret generated successfully',
                    domain: domain,
                    websiteId: website.id
                }, {
                    component: 'webhook-secrets-controller',
                    event: 'secret_generated'
                });
                res.json({
                    secret: secret,
                    instructions: {
                        headers: {
                            'x-wpfort-signature': 'HMAC SHA256 signature of timestamp.payload',
                            'x-wpfort-timestamp': 'Current Unix timestamp in seconds'
                        },
                        example: "\n// PHP Example\n$timestamp = time();\n$payload = json_encode($data);\n$signatureData = $timestamp . '.' . $payload;\n$signature = hash_hmac('sha256', $signatureData, '".concat(secret, "');\n\n$headers = [\n    'x-wpfort-signature: ' . $signature,\n    'x-wpfort-timestamp: ' . $timestamp\n];")
                    }
                });
                return [3 /*break*/, 4];
            case 3:
                error_1 = _a.sent();
                errorDomain = req.params.domain;
                logger_1.logger.error({
                    message: 'Error generating webhook secret',
                    error: error_1,
                    domain: errorDomain
                }, {
                    component: 'webhook-secrets-controller',
                    event: 'secret_generation_error'
                });
                res.status(500).json({ error: error_1.message });
                return [3 /*break*/, 4];
            case 4: return [2 /*return*/];
        }
    });
}); });
// Delete webhook secret
router.delete('/:domain/webhook-secret', function (req, res) { return __awaiter(void 0, void 0, void 0, function () {
    var domain, website, error_2, errorDomain;
    return __generator(this, function (_a) {
        switch (_a.label) {
            case 0:
                _a.trys.push([0, 3, , 4]);
                domain = req.params.domain;
                logger_1.logger.debug({
                    message: 'Deleting webhook secret',
                    domain: domain
                }, {
                    component: 'webhook-secrets-controller',
                    event: 'delete_secret'
                });
                return [4 /*yield*/, (0, db_1.getWebsiteByDomain)(domain)];
            case 1:
                website = _a.sent();
                if (!website) {
                    return [2 /*return*/, res.status(404).json({ error: 'Website not found' })];
                }
                // Delete secret
                logger_1.logger.debug({
                    message: 'Deleting webhook secret',
                    domain: domain,
                    websiteId: website.id
                }, {
                    component: 'webhook-secrets-controller',
                    event: 'delete_secret_request'
                });
                return [4 /*yield*/, webhook_secrets_1.WebhookSecrets.deleteWebhookSecret(website.id)];
            case 2:
                _a.sent();
                logger_1.logger.info({
                    message: 'Webhook secret deleted successfully',
                    domain: domain,
                    websiteId: website.id
                }, {
                    component: 'webhook-secrets-controller',
                    event: 'secret_deleted'
                });
                res.json({ success: true });
                return [3 /*break*/, 4];
            case 3:
                error_2 = _a.sent();
                errorDomain = req.params.domain;
                logger_1.logger.error({
                    message: 'Error deleting webhook secret',
                    error: error_2,
                    domain: errorDomain
                }, {
                    component: 'webhook-secrets-controller',
                    event: 'secret_deletion_error'
                });
                res.status(500).json({ error: error_2.message });
                return [3 /*break*/, 4];
            case 4: return [2 /*return*/];
        }
    });
}); });
exports.default = router;
